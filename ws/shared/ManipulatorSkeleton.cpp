...
amp::ManipulatorState MyManipulator2D::getConfigurationFromIK2Link(const Eigen::Vector2d &end_effector_location, bool first) const
{
    ...
    double sin_theta2 = sign * std::sin(theta2);

    // Calculate the first angle theta1
    double cos_theta1 = (end_effector_location[0]*(a[0]+a[1]*cos_theta2) + sign * end_effector_location[1]*a[1]*sin_theta2)/end_effector_location.squaredNorm();
    double sin_theta1 = (end_effector_location[1]*(a[0]+a[1]*cos_theta2) - sign * end_effector_location[0]*a[1]*sin_theta2)/end_effector_location.squaredNorm();
